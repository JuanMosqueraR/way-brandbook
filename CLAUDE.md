...
- **`entrega-cliente/` construida (24–25/08)** — carpetería completa de assets para la encargada de marketing (logo en todos los formatos, tipografías con itálica real, colores, 6 piezas de empaque + etiqueta tejida por talla). Sticker de Sellado confirmado por Juan (25/08) sin cambios — ya no es un pendiente. **Sigue pendiente:** `git commit`/`push` de esta carpeta (no se hizo todavía, no se pidió explícitamente). Ver Notas Técnicas — Carpeta de Entrega para el detalle, incluido un hallazgo de seguridad en la herramienta MCP de Playwright (no en esta máquina) que hay que tener presente en sesiones futuras.
...
13. **Construir carpeta de entrega para marketing (`entrega-cliente/`)** ✅ construida 24–25/08, Sticker de Sellado confirmado por Juan sin cambios (25/08) — ver Notas Técnicas — Carpeta de Entrega. Falta: hacer `git add`/commit/push de la carpeta (no se hizo todavía, no se pidió explícitamente).
